...
export interface MockStreamEvent {
  type: 'text-delta' | 'tool-call' | 'tool-result' | 'step-start' | 'step-finish' | 'reasoning'
  textDelta?: string
  toolName?: string
  args?: Record<string, unknown>
  result?: unknown
...
export function createMockGenerateText(result?: Partial<MockGenerateTextResult>) {
...
export function createMockStreamText(chunks?: MockStreamEvent[]) {
  ...
  return vi.fn().mockResolvedValue({
    ...
    text: Promise.resolve(defaultChunks.filter(c => c.type === 'text-delta').map(c => c.textDelta).join(''))
  })
}
...
export function createMockAIProvider(name: string = 'openai'): MockAIProvider {
  return {
    name,
...
export function createMockAIService(): MockAIService {
  ...
  return {
    ...
    stream: vi.fn().mockImplementation(async function* () {
      yield { type: 'start' } as StreamChunk
      yield { type: 'content', content: 'Mock streaming response' } as StreamChunk
      yield { type: 'done' } as StreamChunk
    }),
