...
import { mockChat, mockUser, mockMessages } from '../../fixtures'
...
function createMockMessageRepo() {
  return {
    ...
    create: vi.fn().mockResolvedValue({ id: 'new-msg', chatId: mockChat.id, role: 'user', content: 'test' })
  }
}
...
function createMockResponseStrategy() {
  return {
    execute: vi.fn().mockResolvedValue({ content: 'AI response', role: 'assistant' })
  }
}
