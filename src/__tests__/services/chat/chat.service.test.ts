...
import { mockChat, mockChat2, mockUser, mockUser2, mockMessages, mockChatOwnedByUser2 } from '../../fixtures'
...
describe('ChatService', () => {
  ...
  describe('getChatHistory', () => {
    ...
    it('should limit messages when feature flag is false', async () => {
      ...
      expect(mockMessageRepo.findByChatId).toHaveBeenCalledWith(
        mockChat.id,
        { limit: 10 }
      )
    })
  ...
  describe('verifyUserOwnsChat', () => {
    it('should return true when user owns chat', async () => {
      ...
      const result = await chatService.verifyUserOwnsChat(mockChat.id, mockUser.id)
    ...
    it('should return false when user does not own chat', async () => {
      ...
      const result = await chatService.verifyUserOwnsChat(mockChatOwnedByUser2.id, mockUser.id)
    ...
    it('should return false when chat does not exist', async () => {
      ...
      const result = await chatService.verifyUserOwnsChat('non-existent', mockUser.id)
  ...
  describe('getOrCreateChat', () => {
    it('should return existing chat', async () => {
      ...
      const result = await chatService.getOrCreateChat(mockChat.id, mockUser.id)
    ...
    it('should create new chat if not exists', async () => {
      ...
      const result = await chatService.getOrCreateChat(mockChat.id, mockUser.id, 'New Chat')
