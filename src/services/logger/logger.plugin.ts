import { type BoundLogger, LoggerService } from "@utils/logger";
import type {
  FastifyInstance,
  FastifyPluginAsync,
  FastifyReply,
...
const loggerPlugin: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  fastify.log.info(
    "[LoggerPlugin] Structured logging service initialized with response logging"
  );
};
