...
const databasePlugin: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  try {
    ...
    fastify.log.error({ err: error }, '[DatabasePlugin] Failed to connect to database')
    throw error
  }
  ...
  if (config.get('DB_HEALTH_CHECK_ENABLED')) {
    ...
    fastify.log.info({ interval }, '[DatabasePlugin] Starting health check monitoring')
    ...
    healthCheckInterval = setInterval(async () => {
      try {
        ...
        if (!isHealthy) {
          fastify.log.warn('[DatabasePlugin] Health check failed, attempting reconnection...')
  ...
  fastify.addHook('onClose', async (instance) => {
    ...
    try {
      await db.disconnect()
      instance.log.info('[DatabasePlugin] Database connection closed successfully')
    } catch (error) {
      instance.log.error({ err: error }, '[DatabasePlugin] Error closing database connection')
      throw error
    }
  })
